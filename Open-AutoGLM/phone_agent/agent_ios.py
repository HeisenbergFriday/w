...
class IOSPhoneAgent:
    ...
    def _execute_step(
        ...
        # 保存执行动作后的截图
        self._save_screenshot(screenshot, action)
    ...
    def _setup_screenshot_directory(self) -> None:
        """创建截图保存目录."""
        screenshot_dir = Path(self.agent_config.screenshot_dir)
        screenshot_dir.mkdir(parents=True, exist_ok=True)

    def _save_screenshot(self, screenshot, action: dict[str, Any]) -> None:
        """
        保存截图到本地文件.
        
        Args:
            screenshot: Screenshot对象,包含base64数据和尺寸信息
            action: 执行的动作信息
        """
        if not self.agent_config.save_screenshots:
            return
        
        try:
            # 创建任务特定的子目录
            if self._task_start_time is None:
                self._task_start_time = datetime.now().strftime("%Y%m%d_%H%M%S")
            
            task_dir = Path(self.agent_config.screenshot_dir) / self._task_start_time
            task_dir.mkdir(parents=True, exist_ok=True)
            
            # 生成文件名: step_{步骤}_{动作类型}_{时间戳}.png
            action_type = action.get("action", "unknown") if action.get("_metadata") == "do" else "finish"
            timestamp = datetime.now().strftime("%H%M%S")
            filename = f"step_{self._step_count:03d}_{action_type}_{timestamp}.png"
