...
def get_current_app(device_id: str | None = None) -> str:
    ...
    # 使用 'dumpsys window windows' 替代 'dumpsys window'
    # 这个子命令的输出量更小，更容易被 subprocess.run 捕获
    result = subprocess.run(
        adb_prefix + ["shell", "dumpsys", "window", "windows"], capture_output=True, text=True, encoding="utf-8"
    )
    output = result.stdout
    if not output:
        raise ValueError("No output from dumpsys window windows")
