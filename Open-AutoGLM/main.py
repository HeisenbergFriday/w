...
try:
    from load_config import (
        load_config,
        merge_with_env_and_args,
        print_config_summary
    )
    CONFIG_LOADER_AVAILABLE = True
except ImportError as e:
    print(f"⚠️  配置加载模块导入失败: {e}")
    print("将使用默认配置或环境变量")
except Exception as e:
    print(f"⚠️  配置加载出现错误: {e}")
    print("将使用默认配置或环境变量")
...
Examples:
    ...
    # Specify model endpoint (override config.yaml)
    ...
    parser.add_argument(
        ...
        help="Model API base URL (overrides config.yaml)",
    )
...
def main():
    ...
    if CONFIG_LOADER_AVAILABLE:
        ...
        # 如果没有通过命令行参数指定，则从配置文件中获取
        if yaml_config:
            merged_config = merge_with_env_and_args(yaml_config, args)
            
            # 更新 args 对象以使用合并后的配置
            if args.base_url == os.getenv("PHONE_AGENT_BASE_URL", "http://localhost:8000/v1"):
                args.base_url = merged_config['model']['base_url']
            if args.model == os.getenv("PHONE_AGENT_MODEL", "autoglm-phone-9b"):
                args.model = merged_config['model']['model_name']
            if args.apikey == os.getenv("PHONE_AGENT_API_KEY", "EMPTY"):
                args.apikey = merged_config['model']['api_key']
            if args.max_steps == int(os.getenv("PHONE_AGENT_MAX_STEPS", "100")):
                args.max_steps = merged_config['agent']['max_steps']
            if not hasattr(args, 'device_id') or not args.device_id:
                if merged_config['agent']['device_id']:
                    args.device_id = merged_config['agent']['device_id']
            if args.device_type == os.getenv("PHONE_AGENT_DEVICE_TYPE", "adb"):
                args.device_type = merged_config['agent']['device_type']
            if args.lang == os.getenv("PHONE_AGENT_LANG", "cn"):
                args.lang = merged_config['agent']['lang']
            if args.wda_url == os.getenv("PHONE_AGENT_WDA_URL", "http://localhost:8100"):
                args.wda_url = merged_config['ios']['wda_url']
            
            # 添加截图配置
            if not hasattr(args, 'save_screenshots'):
                args.save_screenshots = merged_config['agent'].get('save_screenshots', False)
            if not hasattr(args, 'screenshot_dir'):
                args.screenshot_dir = merged_config['agent'].get('screenshot_dir', './screenshots')
    ...
    if device_type == DeviceType.IOS:
        ...
        agent_config = IOSAgentConfig(
            ...
            screenshot_dir=getattr(args, 'screenshot_dir', './screenshots'),
        )
